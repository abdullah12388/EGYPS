$(function () {
    ...
    document.getElementById("sbmtBTN").addEventListener("click", function () {
        ...
        $.ajax({
            url: "/system/PresalesQuotationToDo/table/api/",
            method: "GET",
            dataType: "json",
            data: {
            ...
            success: function (t) {
                ...
                t.forEach((element) => {
                    // console.log(element);
                    element['details'] = `<button class="btn btn-secondary text-white" style="margin:5px auto;width:100%;" type="button" onclick="ViewRFQModal('${element.RFQ_ID}')"><i class="fa-solid fa-eye me-2"></i> RFQ</button>
                    <button class="btn btn-secondary" style="margin:5px auto;width:100%;" type="button" onclick="ViewQuoteModal('${element.id}')"><i class="fa-solid fa-eye me-2"></i> Quote</button>`;

                    element['actions'] = `<button class="btn btn-success text-white" style="margin:5px auto;width:100%;" type="button" onclick="ViewQuotationApproveModal('${element.id}')"><i class="fa-solid fa-check me-2"></i> Approve</button>
                    <button class="btn btn-danger" style="margin:5px auto;width:100%;" type="button" onclick="ViewQuotationRejectModal('${element.id}')"><i class="fa-solid fa-xmark me-2"></i> Reject</button>`;
                });
                updateTableData(t);
            },
        });
...
function ViewQuoteModal(rowid) {
    ...
    e.bootstrapTable("showLoading");
    $.ajax({
        url: "/system/PresalesQuotationToDo/details/api/",
        method: "GET",
        dataType: "json",
        data: {
            'Quote_ID': rowid,
...
function ViewQuotationApproveModal(rowid){
    var e = $("#pendingQuotationTbl");
    rowData = e.bootstrapTable('getRowByUniqueId', rowid);
    e.bootstrapTable("showLoading");
    console.log(rowData.id);
    document.getElementById('aq_Quotation_ID').innerHTML = rowData.id;
    document.getElementById('aq_RFQ_ID').innerHTML = rowData.RFQ_ID;
    document.getElementById('aq_total_net_value').innerHTML = rowData.net_value;
    document.getElementById('aq_currency').innerHTML = rowData.currency;
    document.getElementById('aq_user').innerHTML = rowData.user;
    document.getElementById('aq_timestamp').innerHTML = rowData.timestamp;
    $('#approve_quote_action_modal').modal({
        backdrop: true,
        keyboard: true,
        focus: true,
        show: true,
...
function ViewQuotationRejectModal(rowid){
    var e = $("#pendingQuotationTbl");
    rowData = e.bootstrapTable('getRowByUniqueId', rowid);
    e.bootstrapTable("showLoading");
    console.log(rowData.id);
    document.getElementById('rq_Quotation_ID').innerHTML = rowData.id;
    document.getElementById('rq_RFQ_ID').innerHTML = rowData.RFQ_ID;
    document.getElementById('rq_total_net_value').innerHTML = rowData.net_value;
    document.getElementById('rq_currency').innerHTML = rowData.currency;
    document.getElementById('rq_user').innerHTML = rowData.user;
    document.getElementById('rq_timestamp').innerHTML = rowData.timestamp;
    $('#reject_quote_action_modal').modal({
        backdrop: true,
        keyboard: true,
        focus: true,
        show: true,
...
function QuotationApproveSubmit(){
    var Quote_ID = document.getElementById('aq_Quotation_ID');
    var approveFeedback = document.getElementById('aq_feedback');
    $.ajax({
        url: "/system/PresalesQuotationToDo/approve/api/",
        method: "GET",
        dataType: "json",
        data: {
            'Quote_ID': Quote_ID.innerHTML,
            'feedback': approveFeedback.value,
        },
        success: function (t) {
            $('#approve_quote_action_modal').modal('hide');
            var e = $("#pendingQuotationTbl");
            e.bootstrapTable("showLoading");
            e.bootstrapTable('removeByUniqueId', Quote_ID.innerHTML);
            e.bootstrapTable("hideLoading");
        },
    });
}

function QuotationRejectSubmit(){
    var Quote_ID = document.getElementById('rq_Quotation_ID');
    var rejectFeedback = document.getElementById('rq_feedback');
    $.ajax({
        url: "/system/PresalesQuotationToDo/reject/api/",
        method: "GET",
        dataType: "json",
        data: {
            'Quote_ID': Quote_ID.innerHTML,
        ...
        success: function (t) {
            $('#reject_quote_action_modal').modal('hide');
            var e = $("#pendingQuotationTbl");
            e.bootstrapTable("showLoading");
            e.bootstrapTable('removeByUniqueId', Quote_ID.innerHTML);
            e.bootstrapTable("hideLoading");
    ...
    .change(function () {
        ...
        o.bootstrapTable("destroy").bootstrapTable({
            ...
            columns: [
                ...
                { field: "net_value", title: "Total Net Value" },
                { field: "currency", title: "Currency" },
                { field: "user", title: "Submitted user" },
...
document.getElementById("print_selected").addEventListener("click", function () {
    var selectedRows = [];
    selectedRows = $("#pendingQuotationTbl").bootstrapTable("getSelections");
    // console.log(selectedRows);
    var printWindow = window.open("", "_blank");
    var currentDate = new Date();
    printWindow.document.open();
    printWindow.document.write(
        "<html><head><title>Pending Quotation Table</title>" +
        "<style>" +
    ...
    var columnNames = [];
    $("#pendingQuotationTbl thead th").each(function () {
